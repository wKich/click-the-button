...
Note: Меня зовут Дмитрий Лазарев. Сегодня я вам расскажу про то как иногда бывает трудно просто кликнуть по кнопке. Сразу оговорюсь, что я не пытаюсь донести до вас какой-то универсальный рецепт решения проблемы кликов по кнопкам, я хочу поделиться историей того с чем мне пришлось столкнутся, когда мне понадобилось просто написать, который просто делает честный клик. Но перед этим пара слов про проект.
...
## 2 unit tests, 0 integration tests TODO
...
Note: Кто уже знаком с папитиром или использует его в своих проектах? Пауза. Puppeteer — представляет удобную абстракцию для управления браузером

Единственным недостатком в нашем случае является то что Puppeteer работает только в nodejs окружении. Нам же необходимо уметь работать с puppeteer из браузера. Поэтому напишем свою небольшую инициализацию.
