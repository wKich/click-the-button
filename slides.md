...
Note: На самом деле существует достаточно много реализаций высокоуровнего API над CDP. Я выбрал Puppeteer из-за того что он предоставляет очень удобное API для работы с DOM. Единственным недостатком в нашем случае является то что Puppeteer внутри себя запускает хром и подключается к нему. Нам же необходимо уметь подключаться к уже запущенному инстансу браузера. Поэтому напишем свою небольшую инициализацию.
