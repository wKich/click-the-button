...
Note: Я работаю в команде Формы контур.экстерна. Как можно догадаться по названию мы делаем формы. Но как и все нормальные разработчики, нам лень заниматься такой рутинной работой, поэтому мы занимается кодогенерацией. Пишем код, который делает формы за нас. В основном это формы отчетности в контролирующие органы: ФНС, РосСтат, ПФР и тд.
...
Note: Поднимите руки те кто знает как его зовут. Пауза. (Что никто не знает? / О, я удивлен что есть те кто его знают) Этого чувака зовут Майк Кон, автор книги "Scrum. Гибкая разработка ПО" и один из соавторов самого Scrum'а. В своей книге он описывает концепт тестовой пирамиды.
...
Note: Будем считать, что AwesomeButton это типичный React компонент в проекте. Он содержит в себе кучу сложной верстки, бизнес-лапшу в методах. И где-то там внутри есть элемент button, по которой мы хотим кликать. Попробуем сделать это использовав Enzyme.
...
Note: Если про CDP никто не знает, то про папитир-то наверно многие слышали? А кто использует? Пауза. Puppeteer — переводится как кукловод и представляет удобную абстракцию для управления браузером. Для нас единственный недостатком является то что он работает только в nodejs окружении. А мы хотим использовать его в браузере. Для этого напишем немного кода.
...
Note: А на любое сообщение по вебсокетам порождается событие, на которое отправляется другое сообщение и так далее. У нас в браузере может быть открыто много вкладок и чтобы выяснить с какой вкладкой нам необходимо работать, нужно применить немного консольной магии
...
Note: Вопросы. Умные таймауты. Почему такая кнопка.